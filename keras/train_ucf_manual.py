...
for i in range(epoch):
    ...
    batches = int(len(file_loc_train)/batch_size)
    for j in tqdm(range(batches)):
        f = file_loc_train[j * batch_size: (j+1)*batch_size]
    ...
    for k, m in tqdm(enumerate(file_loc_valid)):
        ...
        p_loss, p_acc = model_final.evaluate(x_valid, y_valid, verbose = False)
        predicted.append(np.argmax(p_acc))
        Actual.append(np.argmax(y_valid))
        loss.append(p_loss)
    acc_s = accuracy_score(Actual, predicted)
    if acc_s > best_acc:
        model_final.save_weights("model_best_weights.pkl")
        best_acc = acc_s
